...
app.post('/medico/auth/login', async (req, res) => {
  try {
    ...
    console.log(`[MEDICO LOGIN] Proxying to: ${MEDICO}/medico/auth/login`);
    
    const response = await axios.post(`${MEDICO}/medico/auth/login`, req.body, {
      headers: {
        'Content-Type': 'application/json',
        ...req.headers
      },
...
app.get('/medico/auth/me', async (req, res) => {
  try {
    ...
    console.log(`[MEDICO ME] Proxying to: ${MEDICO}/medico/auth/me`);
    
    const response = await axios.get(`${MEDICO}/medico/auth/me`, {
      headers: {
        ...req.headers
      },
      timeout: 30000
    ...
    console.error('[MEDICO ME ERROR]', error.message);
    if (error.response) {
      res.status(error.response.status).json(error.response.data);
    } else {
      res.status(500).json({ 
